...
export async function helpCommand(c: CommandContext<{ Bindings: Env }>) {
  ...
  await sendCommandLog(c.env, username, userId, "/help", command ? `help ${command}` : "invoked");
      ...
      embeds: [
        {
          ...
          fields: [
            ...
            {
              name: "🏇 Đua Ngựa",
              value: "`/duangua` - Uma Musume Racing",
              inline: true,
            },
            {
              name: "🎁 Kiếm Xu Free",
              value: "`/box` - Hộp bí ẩn (8h)\n`/cauca` - Câu cá (90s)",
              inline: true,
            },
            {
              name: "🎰 Gacha System",
              value:
                "`/gacha` - Gacha Blue Archive\n`/banner` - Xem banner rate-up",
              inline: true,
            },
            { name: "📊 Khác", value: "`/top` - Bảng xếp hạng", inline: true },
          ],
          footer: {
            text: "Hoặc dùng /help <command> trực tiếp! Ví dụ: /help gacha",
          },
          ...
          components: [
            {
              ...
              options: [
                ...
                {
                  label: "lucky - Nhận xu hàng ngày",
                  value: "lucky",
                  emoji: { name: "🍀" },
                },
                {
                  label: "taixiu - Tài Xỉu",
                  value: "taixiu",
                  emoji: { name: "🎲" },
                },
                {
                  label: "baucua - Bầu Cua",
                  value: "baucua",
                  emoji: { name: "🎃" },
                },
                {
                  label: "slot - Slot Machine",
                  value: "slot",
                  emoji: { name: "🎰" },
                },
                {
                  label: "duangua - Đua Ngựa",
                  value: "duangua",
                  emoji: { name: "🏇" },
                },
                {
                ...
                {
                  label: "gacha - Gacha System",
                  value: "gacha",
                  emoji: { name: "🎰" },
                },
                {
                  label: "banner - Rate-Up Banner",
                  value: "banner",
                  emoji: { name: "⭐" },
                },
                {
